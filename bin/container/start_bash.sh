...
docker run --rm -it --name="$contNameServer" --cpuset-cpus=0-2 -v ${extConfDir}:/opt/extra-conf-enabled -v ${extSitesConfDir}:/opt/extra-sites-enabled -v ${extDataDir}:/opt/extra-data "$imageName" /bin/bash
