...
# additional directory for instance for static stuff, is mounted to /opt/extra-data
mkdir -p  $destDir/extraMapnikConf
echo <<README4 > $destDir/extraMapnikConf/README.txt
content of this directory can modify mapnik configuration to change the style
or the database connection.

If a files style.xml found will this replace the styles.xml already existing in the
image.
A file named 'datasource-settings.xml.inc' will replace the current connection
settings.
README4
cp $scriptPos/../resources/datasource-settings.xml.inc $destDir/extraMapnikConf/datasource-settings.xml.inc.example
cp $scriptPos/../resources/style.xml $destDir/extraMapnikConf/style.xml.example


# the directory to save the generated tiles 
mkdir -p  $destDir/tiles
